...
public class QuantumFoodSapling extends BaseQuantumSaplingItem {

	public QuantumFoodSapling(Properties properties) {
		super(properties);
	}
	...
	public InteractionResult useOn(UseOnContext ctx) {
		...
		if (!level.isClientSide) {
			...
			if (level.getBlockState(above).getBlock().asItem() == Items.AIR) {
				 ...
				 BlockInit.QUANTUM_FOOD_SAPLING_BLOCK.get().defaultBlockState());
				 
				if (level.getBlockEntity(above) instanceof QuantumSaplingBlockEntity entity) {

					 entity.setSapling(new ItemStack(ItemInit.QUANTUM_FOOD_SAPLING.get()));
