...
public class QuantumGemSapling extends BaseQuantumSaplingItem {

	public QuantumGemSapling(Properties properties) {
		super(properties);
	}
	...
	public InteractionResult useOn(UseOnContext ctx) {
		...
		if (!level.isClientSide) {
			...
			if (level.getBlockState(above).getBlock().asItem() == Items.AIR) {
				 ...
				 BlockInit.QUANTUM_GEM_SAPLING_BLOCK.get().defaultBlockState());
				 
				if (level.getBlockEntity(above) instanceof QuantumSaplingBlockEntity entity) {

					 entity.setSapling(new ItemStack(ItemInit.QUANTUM_GEM_SAPLING.get()));
