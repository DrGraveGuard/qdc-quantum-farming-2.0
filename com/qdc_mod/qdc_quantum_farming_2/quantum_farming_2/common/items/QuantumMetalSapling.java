...
public class QuantumMetalSapling extends BaseQuantumSaplingItem {

	public QuantumMetalSapling(Properties properties) {
		super(properties);
	}
	...
	public InteractionResult useOn(UseOnContext ctx) {
		...
		if (!level.isClientSide) {
			...
			if (level.getBlockState(above).getBlock().asItem() == Items.AIR) {
				 ...
				 BlockInit.QUANTUM_METAL_SAPLING_BLOCK.get().defaultBlockState());
				 
				if (level.getBlockEntity(above) instanceof QuantumSaplingBlockEntity entity) {

					 entity.setSapling(new ItemStack(ItemInit.QUANTUM_METAL_SAPLING.get()));
